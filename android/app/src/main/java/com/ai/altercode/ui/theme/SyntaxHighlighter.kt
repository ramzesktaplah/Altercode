...
object SyntaxHighlighter {
    ...
    // Cache language-specific keyword sets to prevent collection allocations on every keystroke/highlighting pass.
    private val keywordCache: Map<CodeLanguage, Set<String>> = CodeLanguage.entries.associateWith { language ->
        when (language) {
    ...
    // Cache line comment tokens to avoid List allocation per highlighting pass.
    private val lineCommentTokensCache: Map<CodeLanguage, List<String>> = CodeLanguage.entries.associateWith { language ->
        when (language) {
    ...
    private fun keywords(language: CodeLanguage): Set<String> =
        keywordCache[language] ?: commonKeywords

    private fun lineCommentTokens(language: CodeLanguage): List<String> =
        lineCommentTokensCache[language] ?: listOf("//")
