...
object SyntaxHighlighter {
    ...
    // Pre-computed keyword sets per language to avoid object allocation on every highlight invocation.
    // Performance impact: eliminates repeated Set allocations and copying during real-time code editing.
    ...
    // Pre-computed line comment tokens per language to avoid List allocations during tokenization.
    private val languageLineCommentsMap: Map<CodeLanguage, List<String>> = CodeLanguage.entries.associateWith { lang ->
        when (lang) {
    ...
    private fun lineCommentTokens(language: CodeLanguage): List<String> =
        languageLineCommentsMap[language] ?: listOf("//")
